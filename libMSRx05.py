...
# Ported to python3 and some read/write ISO bugs fixed by RcrossF
import time
import binascii
import serial
...
class x05:
    ...
    def __status(self, c):
        ...
        if (c == '2'):
            return "Command misaligned"
        if (c == '4'):
            return "Command invalid"
        if (c == '9'):
            return "Card swiped too fast or too slow"
    ...
    def __expect(self, stri, nonfatal=False):
        ...
        if nonfatal:
            return (a == stri)
        if (a != stri):
            raise RuntimeError(f"communication error; expected [{binascii.hexlify(stri)}] got [{binascii.hexlify(a)}]")
    ...
    def __warn(self):
        if (self.__safetywarnings):
            print(f"WARNING! NEXT SWIPE WILL WRITE TO CARD WITH {'HIGH' if self.getCo() else 'LOW'} COERCIVITY")

    def getFirmwareVersion(self):
        self.__s.write('\x1B\x76'.encode())
        time.sleep(.1)
        self.__expect(ESC)
        version = ""
        while (self.__s.inWaiting()):
            version += self.__s.read().decode()
        return version

    def getDeviceModel(self):
        self.__s.write('\x1B\x74'.encode())
        time.sleep(.1)
        self.__expect(ESC)
        version = ""
        while (self.__s.inWaiting()):
    ...
    def setLED(self, bits):  # R3 Y2 G1
        ...
        if (bits == 7):
            self.__s.write('\x1B\x82'.encode())
        if (bits == 0):
            self.__s.write('\x1B\x81'.encode())
        if (bits == 1):
            self.__s.write('\x1B\x83'.encode())
        if (bits == 2):
            self.__s.write('\x1B\x84'.encode())
        if (bits == 4):
            self.__s.write('\x1B\x85'.encode())
    ...
    def setLZ(self, track):
        try:
            ...
            pass
        self.__s.write(('\x1B\x7A'+chr(track[0])+chr(track[1])).encode())
        return self.__expect(ACK, True)

    def getLZ(self):
        self.__s.write('\x1B\x6C'.encode())
        self.__expect('\x1B')
    ...
    def eraseTracks(self, tracks, coerc=1):
        ...
        self.__s.write(('\x1B\x63'+chr(select)).encode())
    ...
    def setBPC(self, bpc):
        ...
        self.__s.write(('\x1B\x6F'+chr(bpc[0])+chr(bpc[1])+chr(bpc[2])).encode())
        self.__expect(ACK)
    ...
    def setBPI(self, bpi):
        ...
        if (failed):
            raise Exception(
                "Please specify BPI (high(1) or low(0), or as-is(None)) for all 3 tracks")
        result = True
        for i in [0, 1, 2]:
    ...
    def __setBPI(self, track, bpi):
        if (track == 1 and bpi == 0):
            code = '\xA0'
        elif (track == 1 and bpi == 1):
            code = '\xA1'
        elif (track == 2 and bpi == 0):
            code = '\x4B'
        elif (track == 2 and bpi == 1):
            code = '\xD2'
        elif (track == 3 and bpi == 0):
            code = '\xC0'
        elif (track == 3 and bpi == 1):
            code = '\xC1'
        else:
            code = '\xFF'

        self.__s.write(('\x1B\x62'+code).encode())
        return self.__expect(ACK, True)

    # you should hear a click when changing coercivity

    def setHiCo(self):
        self.__s.write('\x1B\x78'.encode())
        return self.__expect(ACK, True)

    def setLoCo(self):
        self.__s.write('\x1B\x79'.encode())
        return self.__expect(ACK, True)

    def getCo(self):
        self.__s.write('\x1B\x64'.encode())
        r = self.__s.read(2).decode()
        if (r == ESC+'h'):
            return 1
        if (r == ESC+'l'):
            return 0
    ...
    # Returns 2 lists, first is the track data, second is the track status
    # Status is 0 for success, * for read error, + for empty track. The error codes are not consistent and empty tracks seem to report read errors
    def readISO(self):
        self.reset()
        self.__s.write('\x1B\x72'.encode())
        data = ['', '', '']
        status = [0, 0, 0]
        ...
        for track in [0, 1, 2]:
            ...
            data[track] = self.__read_until(ESC.encode())
            if (len(data[track]) == 0):
                status[track] = self.__s.read().decode()
                if (track == 2):
    ...
    def writeISO(self, data):
        ...
        for i in [0, 1, 2]:
            ...
            if (len(data[i]) > 0):
                ...
                if (len(data[i]) < 2 or data[i][0] != (';' if i else '%') or data[i][-1] != '?'):
                    raise Exception(f"Track {chr(0x30+1+i)} is not ISO formatted")
                else:
                    data[i] = data[i][1:-1]
        ...
        for i in [0, 1, 2]:
            if not set(data[i]).issubset(str23 if i else str1):
                raise Exception("Track"+chr(0x30+1+i) +
                                " contains illegal characters")

        command = ESC+'\x77'+ESC+'\x73'
        for i in [i for i, str in enumerate(data) if len(str) > 0]: # If there's a problem it's probably here
        #for i in [0, 1, 2]:
            command += ESC+chr(i+1)+data[i]
        command += '?\x1C'

        self.__s.write(command.encode())

        # My reader/writer always returns an error on write even if the card is written successfully
        # Not sure how other devices handle it
        #self.__expect(ESC.encode())
        

        # check status
        status = self.__s.read().decode()
        if (status == '0'):
            return True
        else:
    ...
    def __reverseStringBits(self, text):
        ...
        for ti in range(len(text)):
            rev += chr(sum(1 << (7-i)
                       for i in range(8) if ord(text[ti]) >> i & 1))
        return rev
    ...
    def writeRaw(self, data, reversed=True):
        ...
        for i in [0, 1, 2]:
            command += ESC + \
                chr(i+1)+chr(len(data[i])) + \
                (self.__reverseStringBits(data[i]) if reversed else data[i])
        command += '?\x1C'
